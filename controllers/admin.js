...
exports.postAddStudent = async (req, res, next) => {
  ...
  const hashedPassword = await hashing(password);
  const sql1 =
    'select count(*) as `count`, section from student where section = (select max(section) from student where dept_id = ? group by section) AND dept_id = ? group by section';
  const results = await queryParamPromise(sql1, [department, department]);
  let section = 1;
  if (results[0]!=null && results[0].count !== 0) {
