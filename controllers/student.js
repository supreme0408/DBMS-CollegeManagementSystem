...
exports.getFeeHistory = async (req,res,next) =>{
  const sql1 = 'SELECT * FROM student WHERE s_id = ?';
  const studentData = (await queryParamPromise(sql1, [req.user]))[0];
  const stu_id=studentData.s_id;
  const sql2 = 'SELECT * FROM fee WHERE s_id = ?';
  const feeData = (await queryParamPromise(sql2, [req.user]))[0];
  console.log(feeData);
  res.render('Student/feehistory',{
    page_name: 'Fee History',
    studentData,
    feeData,
  });
};
